set.seed(47)
rpap_data = geoR::grf(40, cov.pars = c(1, 1), nugget = 0.1,
              messages = FALSE)
y = rpap_data$data
...
geoR_ml_beta = geoR::krige.conv(rpap_data, krige = kc_ml,
                                locations = cbind(1, 1),
                                output = geoR_out)$beta.est
kc_reml = geoR::krige.control(obj.model = geoR_reml)
geoR_reml_beta = geoR::krige.conv(rpap_data, krige = kc_reml,
                                  locations = cbind(1, 1),
                                  output = geoR_out)$beta.est

# save output
fpath = system.file("testdata",  package = "gear")
fname = paste(fpath, "/ploglik_cmodStd_r_psill_angle_par3.rda", sep = "")
