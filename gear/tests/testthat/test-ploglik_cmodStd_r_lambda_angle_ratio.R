fpath = system.file("testdata",
                    "ploglik_cmodStd_r_lambda_angle_ratio.rda",
                    package = "gear")
load(fpath)
...
gear_ml = optimx::optimx(par = c(1, 1, pi/8, 1/1.5),
                         fn = ploglik_cmodStd_r_lambda_angle_ratio,
                         lower = c(0.001, 0, 0, 0.001),
...
sigmasq_ml = ploglik_cmodStd_r_lambda_angle_ratio(with(gear_ml, c(p1, p2, p3, p4)),
                                                       x = x,
                                                       y = y,
                                                       d = d,
                                                       weights = weights,
...
gear_reml = optimx::optimx(par = c(1, 1, pi/8, 1/1.5),
                           ...
                           reml = TRUE,
                           control = list(dowarn = FALSE))
sigmasq_reml = ploglik_cmodStd_r_lambda_angle_ratio(with(gear_ml, c(p1, p2, p3, p4)),
                                                       x = x,
                                                       y = y,
                                                       d = d,
                                                       weights = weights,
...
test_that("ploglik_cmodStd_r_lambda_angle_ratio accuracy (geoR)", {
  expect_true(abs(geoR_ml$cov.pars[1] - sigmasq_ml) < 1e-1)
  expect_true(abs(geoR_ml$cov.pars[2] - gear_ml$p1 * gear_ml$p4) < 1e-3)
  ...
  expect_true(abs(geoR_reml$cov.pars[2] - gear_reml$p1 * gear_reml$p4) < 1e-3)
...
object_ml = estimate(object, method = "L-BFGS-B",
                     lower = list(r = 0.001, lambda = 0, angle = 0, ratio = 0.001),
                     upper = list(r = 2, lambda = 2, angle = pi - .001, ratio = 1),
...
test_that("estimate r_lambda_angle_ratio accuracy (geoR)", {
  ...
  expect_true(max(abs(gear_ml[1:5] - object_ml$optimx[1:5])) < 1e-4)
...
test_that("estimate w/ and w/o radians r_lambda_angle_ratio accuracy", {
