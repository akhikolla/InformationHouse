fpath = system.file("testdata",
                    "ploglik_cmodStd_r_lambda_angle_par3.rda",
                    package = "gear")
load(fpath)
...
gear_ml = optimx::optimx(par = c(1, 1, pi/8, 1), 
                         fn = ploglik_cmodStd_r_lambda_angle_par3, 
                         lower = c(0.001, 0, 0, 0.001),
                         upper = c(2, 2, pi - 0.001, 2.5),
                         method = "L-BFGS-B", 
                         x = x, y = y, d = d,  
                         weights = weights, scmod = scmod,
                         reml = FALSE,
                         control = list(kkt = FALSE, dowarn = FALSE))
sigmasq_ml = ploglik_cmodStd_r_lambda_angle_par3(with(gear_ml, c(p1, p2, p3, p4)),
                                                       x = x, 
                                                       y = y,
                                                       d = d,
                                                       weights = weights,
...
gear_reml = optimx::optimx(par = c(1, 1, pi/8, 1), 
                           fn = ploglik_cmodStd_r_lambda_angle_par3, 
                           lower = c(0.001, 0, 0, 0.001),
                           upper = c(15, 5, pi - 0.001, 2.5),
                           method = "L-BFGS-B", 
                           x = x, y = y, d = d, nugget = geoR_reml$nugget, 
                           weights = weights, scmod = scmod,
                           reml = TRUE,
                           control = list(kkt = FALSE, dowarn = FALSE))
sigmasq_reml = ploglik_cmodStd_r_lambda_angle_par3(with(gear_ml, c(p1, p2, p3, p4)),
                                                       x = x, 
                                                       y = y,
                                                       d = d,
                                                       weights = weights,
...
test_that("ploglik_cmodStd_r_lambda_angle_par3 accuracy (geoR)", {
  ...
  expect_true(abs(geoR_reml$cov.pars[1] - sigmasq_reml) < 1e-2)
  expect_true(abs(geoR_reml$cov.pars[2] - gear_reml$p1 / geoR_reml$aniso.pars[2]) < 1e-4)
...
object_ml = estimate(object, method = "L-BFGS-B",
                     lower = list(r = 0.001, lambda = 0, angle = 0, par3 = 0.001),
                     upper = list(r = 2, lambda = 2, angle = pi - 0.001, par3 = 2.5),
...
object_reml = estimate(object, method = "L-BFGS-B",
                       lower = list(r = 0.001, lambda = 0, angle = 0, par3 = 0.001),
                       upper = list(r = 15, lambda = 5, angle = pi - .001, par3 = 2.5),
                       est_nugget = TRUE, est_par3 = TRUE,
                       est_angle = TRUE, est_ratio = FALSE,
                       verbose = FALSE, reml = TRUE)

test_that("estimate r_lambda_angle_par3 accuracy (geoR)", {
  ...
  expect_true(max(abs(gear_ml[1:5] - object_ml$optimx[1:5])) < 1e-5)
...
test_that("estimate w/ and w/o radians r_lambda_angle_par3 accuracy", {
