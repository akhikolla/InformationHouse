fpath = system.file("testdata",
                    "ploglik_cmodStd_r_lambda_ratio_par3.rda",
                    package = "gear")
load(fpath)
...
gear_ml = optimx::optimx(par = c(1, 1, 1/1.5, 1), 
                         fn = ploglik_cmodStd_r_lambda_ratio_par3, 
                         lower = c(0.001, 0, 0.001, 0.001),
                         upper = c(2, 2, 1, 2.5),
                         method = "L-BFGS-B", 
                         x = x, y = y, d = d,  
...
sigmasq_ml = ploglik_cmodStd_r_lambda_ratio_par3(with(gear_ml, c(p1, p2, p3, p4)),
                                                       x = x, 
                                                       y = y,
                                                       d = d,
                                                       weights = weights,
...
gear_reml = optimx::optimx(par = c(1, 1, 1/1.5, 1), 
                           ...
                           reml = TRUE,
                           control = list(dowarn = FALSE))
sigmasq_reml = ploglik_cmodStd_r_lambda_ratio_par3(with(gear_ml, c(p1, p2, p3, p4)),
                                                       x = x, 
                                                       y = y,
                                                       d = d,
                                                       weights = weights,
...
test_that("ploglik_cmodStd_r_lambda_ratio_par3 accuracy (geoR)", {
  expect_true(abs(geoR_ml$cov.pars[1] - sigmasq_ml) < 1e-1)
  expect_true(abs(geoR_ml$cov.pars[2] - gear_ml$p1 * gear_ml$p3) < 1e-3)
  ...
  expect_true(abs(geoR_reml$cov.pars[2] - gear_reml$p1 * gear_reml$p3) < 1e-3)
...
object_ml_radians = estimate(object_radians, method = "L-BFGS-B",
                             lower = list(r = 0.001, lambda = 0, ratio = 0.001, par3 = 0.001),
                             upper = list(r = 2, lambda = 2, ratio = 1, par3 = 2.5),
                             est_nugget = TRUE, est_par3 = TRUE,
                             est_angle = FALSE, est_ratio = TRUE,
                             verbose = FALSE)

test_that("estimate w/ and w/o radians r_lambda_ratio_par3 accuracy", {
