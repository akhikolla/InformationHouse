fpath = system.file("testdata",
                    "ploglik_cmodStd_r_psill_angle.rda",
                    package = "gear")
load(fpath)
...
gear_ml = optimx::optimx(par = c(1, 1, pi/8), 
                         fn = ploglik_cmodStd_r_psill_angle, 
                         lower = c(0.001, 0.001, 0),
                         upper = c(2, 2, pi - 0.001),
                         method = "L-BFGS-B", 
...
test_that("ploglik_cmodStd_r_psill_angle accuracy (geoR)", {
  expect_true(abs(geoR_ml$cov.pars[1] - gear_ml$p2) < 1e-4)
  expect_true(abs(geoR_ml$cov.pars[2] - gear_ml$p1/geoR_ml$aniso.pars[2] ) < 1e-4)
  expect_true(abs(geoR_ml$aniso.pars[1] - gear_ml$p3) < 1e-4)
  expect_equal(geoR_ml$loglik, gear_ml$value/-2)
  expect_true(abs(geoR_reml$cov.pars[1] - gear_reml$p2) < 1e-4)
  expect_true(abs(geoR_reml$cov.pars[2] - gear_reml$p1/geoR_reml$aniso.pars[2]) < 1e-2)
...
object_ml = estimate(object, method = "L-BFGS-B",
                     ...
                     upper = list(r = 2, psill = 2, angle = pi - .001, par3 = 2.5),
...
test_that("estimate r_psill_angle accuracy (geoR)", {
  expect_true(abs(geoR_ml$cov.pars[1] - object_ml$mod$psill) < 1e-2)
  expect_true(abs(geoR_ml$cov.pars[2] - object_ml$mod$r/geoR_ml$aniso.pars[2]) < 1e-4)
  ...
  expect_true(abs(geoR_reml$cov.pars[2] - object_reml$mod$r/geoR_ml$aniso.pars[2]) < 1e-4)
...
object_ml_radians = estimate(object_radians, method = "L-BFGS-B",
                             lower = list(r = 0.001, psill = 0.001, angle = 0),
                             upper = list(r = 2, psill = 2, angle = (pi - .001) * 180/pi),
                             est_nugget = FALSE, est_par3 = FALSE,
                             est_angle = TRUE, est_ratio = FALSE,
                             verbose = FALSE)

test_that("estimate w/ and w/o radians r_psill_angle accuracy", {
