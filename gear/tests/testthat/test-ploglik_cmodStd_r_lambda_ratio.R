fpath = system.file("testdata",
                    "ploglik_cmodStd_r_lambda_ratio.rda",
                    package = "gear")
load(fpath)
...
gear_ml = optimx::optimx(par = c(1, 1, 1/1.5), 
                         fn = ploglik_cmodStd_r_lambda_ratio, 
...
sigmasq_ml = ploglik_cmodStd_r_lambda_ratio(with(gear_ml, c(p1, p2, p3)),
                                                       x = x, 
                                                       y = y,
                                                       d = d,
                                                       weights = weights,
...
gear_reml = optimx::optimx(par = c(1, 1, 1/1.5), 
                           ...
                           reml = TRUE,
                           control = list(dowarn = FALSE))
sigmasq_reml = ploglik_cmodStd_r_lambda_ratio(with(gear_ml, c(p1, p2, p3)),
                                                       x = x, 
                                                       y = y,
                                                       d = d,
                                                       weights = weights,
...
test_that("ploglik_cmodStd_r_lambda_ratio accuracy (geoR)", {
...
test_that("estimate w/ and w/o radians r_lambda_ratio accuracy", {
