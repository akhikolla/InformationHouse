fpath = system.file("testdata",
                    "ploglik_cmodStd_r_lambda_angle_ratio_par3.rda",
                    package = "gear")
load(fpath)
...
gear_ml = optimx::optimx(par = c(1, 1, pi/8, 1/1.5, 1), 
                         fn = ploglik_cmodStd_r_lambda_angle_ratio_par3, 
                         lower = c(0.001, 0, 0, 0.001, 0.001),
                         upper = c(2, 2, pi - 0.001, 1, 2.5),
                         method = "L-BFGS-B", 
                         x = x, y = y, d = d,  
...
sigmasq_ml = ploglik_cmodStd_r_lambda_angle_ratio_par3(with(gear_ml, c(p1, p2, p3, p4, p5)),
                                                       x = x, 
                                                       y = y,
                                                       d = d,
                                                       weights = weights,
...
sigmasq_reml = ploglik_cmodStd_r_lambda_angle_ratio_par3(with(gear_ml, c(p1, p2, p3, p4, p5)),
                                                       x = x, 
                                                       y = y,
                                                       d = d,
                                                       weights = weights,
...
test_that("ploglik_cmodStd_r_lambda_angle_ratio_par3 accuracy (geoR)", {
  expect_true(abs(geoR_ml$cov.pars[1] - sigmasq_ml) < 1e-1)
  ...
  expect_true(abs(geoR_ml$aniso.pars[2] - 1/gear_ml$p4) < 1e-1)
  expect_true(abs(geoR_ml$kappa - gear_ml$p5) < 1e-1)
  expect_true(abs(geoR_ml$loglik - gear_ml$value/-2) < 1e-3)
  expect_true(abs(geoR_reml$cov.pars[1] - sigmasq_reml) < 1e-1)
  ...
  expect_true(abs(geoR_reml$aniso.pars[2] - 1/gear_reml$p4) < 1e-1)
  expect_true(abs(geoR_reml$kappa - gear_reml$p5) < 1e-1)
...
object_ml = estimate(object, method = "L-BFGS-B",
                     ...
                     upper = list(r = 2, lambda = 2, angle = pi - 0.001, ratio = 1, par3 = 2.5),
                     est_nugget = TRUE, est_par3 = TRUE,
                     est_angle = TRUE, est_ratio = TRUE,
                     verbose = FALSE)
object_reml = estimate(object, method = "L-BFGS-B",
                       lower = list(r = 0.001, lambda = 0, angle = 0, ratio = 0.001, par3 = 0.001),
                       upper = list(r = 15, lambda = 5, angle = pi - 0.001, ratio = 1, par3 = 2.5),
                       est_nugget = TRUE, est_par3 = TRUE,
                       est_angle = TRUE, est_ratio = TRUE,
                       verbose = FALSE, reml = TRUE)

test_that("estimate r_lambda_angle_ratio_par3 accuracy (geoR)", {
  ...
  expect_true(max(abs(gear_ml[1:6] - object_ml$optimx[1:6])) < 1e-4)
  ...
  expect_equivalent(gear_reml[1:6], object_reml$optimx[1:6])
...
test_that("estimate w/ and w/o radians r_lambda_angle_ratio_par3 accuracy", {
