fun_ahryptwo <- function(oy, od, oz, best, tau, alpha, repnum, ...) {
    ...
    B1 <- oy[1] * a0 + t(c(oy[2:ntl], tau) - oy[1:ntl]) %*% a[1:ntl, ]
    ...
    upp = exp(ca3 * stmb/sqrt(n)/tau/avv) * avv
    low = exp(-ca3 * stmb/sqrt(n)/tau/avv) * avv
