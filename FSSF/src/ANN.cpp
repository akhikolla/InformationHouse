...
void annDeallocPts(ANNpointArray &pa, ANNidx n)			// deallocate points
{
    for (int i=0; i<n; ++i) {
        delete [] pa[i];
    }
...
{
    if (level == ANNabort) {
        Rcpp::Rcerr << "ANN: ERROR------->" << msg << "<-------------ERROR\n";
        Rcpp::stop("Stopped running.\n");
    }
    else {
        Rcpp::Rcerr << "ANN: WARNING----->" << msg << "<-------------WARNING\n";
    }
}
