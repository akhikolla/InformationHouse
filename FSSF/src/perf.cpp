...
{
    Rcpp::Rcout << title << "= [ ";
	Rcpp::Rcout.width(9); Rcpp::Rcout << s.mean()/div			<< " : ";
	Rcpp::Rcout.width(9); Rcpp::Rcout << s.stdDev()/div		<< " ]<";
	Rcpp::Rcout.width(9); Rcpp::Rcout << s.min()/div			<< " , ";
	Rcpp::Rcout.width(9); Rcpp::Rcout << s.max()/div			<< " >\n";
}

DLL_API void annPrintStats(				// print statistics for a run
	ANNbool validate)					// true if average errors desired
{
	Rcpp::Rcout.precision(4);					// set floating precision
	Rcpp::Rcout << "  (Performance stats: "
		 << " [      mean :    stddev ]<      min ,       max >\n";
	...
	Rcpp::Rcout.precision(0);					// restore the default
	Rcpp::Rcout << "  )\n";
	Rcpp::Rcout.flush();
}
