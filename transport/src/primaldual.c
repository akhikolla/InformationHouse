/*
   ...
   $Revision: 1.6 $   $Date: 2013/09/08 14:41:00 $
...
void primaldual(int *d, int *rmass, int *cmass, int *numr, int *numc, int *flowmatrix)
{
   int i,j; /* indices */
   int n1,n2;
...
/* The main loop */
   while(feasible == 0) {
      ...
      maxflow(&state);
      if (arrayisum(state.rowsurplus, n1) > 0) {
...
/* ------------ Functions called by primaldual ------------------------- */
...
void initvalues(State *state) {
   ...
   for (i = 0; i < n1; i++) {
      ...
      state->dualu[i] = arrayimin(state->colhelper, n2);
   }
   for (j = 0; j < n2; j++) {
      for (i = 0; i < n1; i++) 
	 state->rowhelper[i] = COST(i, j, state, n1) - state->dualu[i];
      state->dualv[j] = arrayimin(state->rowhelper, n1);
   }
   for (i = 0; i < n1; i++) {
      for (j = 0; j < n2; j++) {
...
void updateduals(State *state) 
{
   ...
   mini = arrayimin(state->collectvals, count);
   for (i = 0; i < n1; i++) {
     if (state->rowlab[i] != -1)
       state->dualu[i] += mini;
   }
