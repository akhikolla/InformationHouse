// Generated by using Rcpp::compileAttributes() -> do not edit by hand
// Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

#include <Rcpp.h>

using namespace Rcpp;

// rcpp_ksNN
List rcpp_ksNN(NumericVector Label, NumericVector Distance, double L_C);
RcppExport SEXP _ksNN_rcpp_ksNN(SEXP LabelSEXP, SEXP DistanceSEXP, SEXP L_CSEXP) {
BEGIN_RCPP
    Rcpp::RObject rcpp_result_gen;
    Rcpp::RNGScope rcpp_rngScope_gen;
    Rcpp::traits::input_parameter< NumericVector >::type Label(LabelSEXP);
    Rcpp::traits::input_parameter< NumericVector >::type Distance(DistanceSEXP);
    Rcpp::traits::input_parameter< double >::type L_C(L_CSEXP);
    rcpp_result_gen = Rcpp::wrap(rcpp_ksNN(Label, Distance, L_C));
    return rcpp_result_gen;
END_RCPP
}

static const R_CallMethodDef CallEntries[] = {
    {"_ksNN_rcpp_ksNN", (DL_FUNC) &_ksNN_rcpp_ksNN, 3},
    {NULL, NULL, 0}
};

RcppExport void R_init_ksNN(DllInfo *dll) {
    R_registerRoutines(dll, NULL, CallEntries, NULL, NULL);
    R_useDynamicSymbols(dll, FALSE);
}
